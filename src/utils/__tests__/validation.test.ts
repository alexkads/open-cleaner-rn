...
export const isValidCleaningType = (type: string): type is 'quick' | 'deep' | 'custom' => {
  return ['quick', 'deep', 'custom'].includes(type);
};

export const isValidStatus = (status: string): status is 'success' | 'warning' | 'error' => {
...
export const calculateSpacePercentage = (spaceFreed: number, totalSpace: number): number => {
...
export const sanitizeErrorMessage = (error: string): string => {
  ...
  return error.replace(/\/Users\/[^\/\s]+/g, '/Users/***')
...
describe('Validation Utils', () => {
  ...
  describe('sanitizeErrorMessage', () => {
    test('should sanitize user paths', () => {
      ...
      expect(sanitizeErrorMessage(error)).toBe('Error in /Users/***/project/file.js');
    });

    test('should sanitize home paths', () => {
      const error = 'Error in /home/jane/project/file.js';
      expect(sanitizeErrorMessage(error)).toBe('Error in /home/***/project/file.js');
    });

    test('should handle multiple paths', () => {
      const error = 'Error copying from /Users/john/src to /Users/jane/dst';
      expect(sanitizeErrorMessage(error)).toBe('Error copying from /Users/***/src to /Users/***/dst');
    });
