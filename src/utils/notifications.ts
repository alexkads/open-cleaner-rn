...
export const notifications = {
  // Scan notifications
  scanStart: () => toast.loading('Iniciando scan do sistema...', {
    description: 'Procurando por arquivos cache e temporários',
    duration: Infinity
  }),
  
  scanProgress: (taskName: string, current: number, total: number, spaceFound: string, toastId?: string | number) => 
    toast.loading(`Scaneando ${taskName}...`, {
      id: toastId,
      description: `${current}/${total} - ${spaceFound} encontrados`
    }),
  
  scanSuccess: (spaceFound: string, categoriesCount: number, toastId?: string | number) =>
  ...
  // Clean notifications
  cleanStart: (categoriesCount: number) => toast.loading('Iniciando limpeza...', {
    description: `${categoriesCount} categorias serão limpas`,
    duration: Infinity
  }),
  
  cleanProgress: (taskName: string, current: number, total: number, spaceFreed: string, toastId?: string | number) =>
    toast.loading(`Limpando ${taskName}...`, {
      id: toastId,
      description: `${current}/${total} - ${spaceFreed} liberados`
    }),
  
  cleanSuccess: (spaceFreed: string, filesDeleted: number, duration: string, toastId?: string | number) =>
  ...
  cleanWarning: (spaceFreed: string, filesDeleted: number, duration: string, errorsCount: number, toastId?: string | number) =>
  ...
  systemReady: () => toast.success('Sistema inicializado com sucesso!', {
    description: 'Clean RN está pronto para otimizar seu ambiente de desenvolvimento',
    duration: 3000
  }),
  
  systemError: (error: string) => toast.error('Falha na inicialização do sistema', {
    description: `Alguns recursos podem não funcionar corretamente: ${error}`,
    duration: 5000
  }),

  // Debug notifications
  debugStart: () => toast.loading('Executando diagnósticos...', {
    description: 'Testando conexões e funcionalidades',
    duration: Infinity
  }),
  
  debugSuccess: (successCount: number, totalTests: number, toastId?: string | number) =>
  ...
  debugWarning: (successCount: number, totalTests: number, toastId?: string | number) =>
  ...
  debugError: (successCount: number, totalTests: number, toastId?: string | number) =>
  ...
  // Info notifications
  logsToConsole: () => toast.info('Logs enviados para o console', {
