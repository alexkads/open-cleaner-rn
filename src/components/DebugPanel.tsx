...
export default function DebugPanel({ isVisible, onClose, debugInfo }: DebugPanelProps) {
  ...
  const runDiagnostics = async () => {
    ...
    const toastType = isAllSuccess ? 'success' : successCount > 0 ? 'warning' : 'error';
    const toastTitle = isAllSuccess ? 'Todos os testes passaram!' : 
                      successCount > 0 ? 'Alguns testes falharam' : 
        ...
        >
          ...
          >
            ...
            <div className="space-y-6">
              {/* Current State */}
              <div>
                <h3 className="text-lg font-semibold mb-3 text-primary">Current State</h3>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div className="space-y-2">
                    <p>Scanning: <span className={debugInfo.isScanning ? 'text-yellow-500' : 'text-green-500'}>{debugInfo.isScanning ? 'Yes' : 'No'}</span></p>
                    <p>Cleaning: <span className={debugInfo.isCleaning ? 'text-yellow-500' : 'text-green-500'}>{debugInfo.isCleaning ? 'Yes' : 'No'}</span></p>
                    <p>Total Space Found: <span className="text-primary">{formatBytes(debugInfo.totalSpaceFound)}</span></p>
                  </div>
                  <div className="space-y-2">
                    <p>Total Tasks: <span className="text-primary">{debugInfo.tasksCount}</span></p>
                    <p>Cleanable Tasks: <span className="text-primary">{debugInfo.cleanableTasks}</span></p>
                    {debugInfo.lastError && (
                      <p className="text-red-500 text-xs break-all">Last Error: {debugInfo.lastError}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              {/* System Tests */}
              <div>
                <h3 className="text-lg font-semibold mb-3 text-primary">System Tests</h3>
              ...
              {/* Actions */}
              <div>
                <h3 className="text-lg font-semibold mb-3 text-primary">Debug Actions</h3>
              ...
              <div>
                <h3 className="text-lg font-semibold mb-3 text-primary">Clean Button Analysis</h3>
                <div className="space-y-2 text-sm">
                  <div className="p-3 bg-white/5 rounded-lg">
                    <p className="font-medium mb-2">Button should be enabled when:</p>
                    <ul className="space-y-1 text-gray-300">
                      <li>• Not scanning: {debugInfo.isScanning ? '❌' : '✅'}</li>
                      <li>• Not cleaning: {debugInfo.isCleaning ? '❌' : '✅'}</li>
                      <li>• Has space found: {debugInfo.totalSpaceFound > 0 ? '✅' : '❌'}</li>
                      <li>• Has cleanable tasks: {debugInfo.cleanableTasks > 0 ? '✅' : '❌'}</li>
                    </ul>
              ...
              {/* Task Status Details */}
              <div>
                <h3 className="text-lg font-semibold mb-3 text-primary">Task Status Details</h3>
                <div className="space-y-2 text-xs">
                  <div className="p-3 bg-white/5 rounded-lg max-h-40 overflow-y-auto">
                    ...
                    <div className="space-y-1 font-mono">
                      <div>Total Tasks: <span className="text-primary">{debugInfo.tasksCount}</span></div>
                      <div>Space Found: <span className="text-primary">{formatBytes(debugInfo.totalSpaceFound)}</span></div>
                      <div>Cleanable: <span className="text-primary">{debugInfo.cleanableTasks}</span></div>
                    </div>
